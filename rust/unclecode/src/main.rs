...
fn run_native_rust_command(args: &[OsString], started_at: Instant) -> Result<u8, String> {
    match args.first().and_then(|arg| arg.to_str()) {
        ...
        _ => Err("Usage: unclecode rust <queue-smoke|session-smoke|run -- <program> [args...]|shell -- <command>|queue <push|pop|list|len|clear>|session <persist|list>|auth <status|resolve|inspect-oauth-token|authorization-url|parse-callback|request-spec|request-body|parse-token-response|parse-device-response|parse-codex-device-response|parse-codex-token-response|read-credentials|write-raw|save-api-key|logout>|model <openai-registry|openai-reasoning|price|estimate-cost|detect-provider|provider-route|provider-route-json|provider-runtime-json|capability|catalog|label>|provider <system-prompt|openai-request-spec|openai-request-spec-json|openai-post|openai-chat-body|openai-codex-body|reasoning-effort|app-reasoning|openai-chat-tools|openai-query-messages|openai-chat-response|openai-chat-response-json|openai-tool-actions|openai-chat-query|openai-chat-complete|request-error|loop-decision|iteration-action-plan|loop-limit|turn-step|complete-turn-step|tool-dispatch-plan|reset-state|runtime-settings|append-state|start-turn|attachment-caps|tool-result-turn-step|reasoning-delta|reasoning-delta-record|route-trace|turn-started-trace|calling-trace|turn-completed-trace|openai-responses-message|tool-trace-started|tool-trace-completed|tool-execution-start|tool-execution-result|tool-execution-finish|tool-execution-finish-result|tool-result|tool-result-container|tool-result-turn-entries|openai-user-message|openai-assistant-message|openai-tool-message|openai-responses-input|openai-responses-tools|gemini-request-spec|gemini-request-spec-json|gemini-post|gemini-query-messages|gemini-tools|gemini-user-content|gemini-function-response|gemini-response|gemini-generate-request|anthropic-request-spec|anthropic-request-spec-json|anthropic-post|anthropic-query-messages|anthropic-user-message|anthropic-tool-result|anthropic-response|anthropic-messages-request>|command <route|help>|context <guidance|repo-map|repo-map-token|worktree-fingerprint|freshness|selection|token-budget|estimate-tokens|hotspots|diff|skills|skill-load|auth-issues>|orchestrator <classify-intent|complex-tasks|parse-plan-response|worker-budget|planner-prompt|guardian-review-prompt|synthesis-prompt|changed-files|trace-event>|team <run-config|worker-options|lanes|worker-spawn-args|dispatch-status|child-env|worker-close-outcome|list-runs>|ux <panel>|work-runtime <parse-args|build-command-args|with-cwd|entrypoint-paths>|http <post|proxy-policy>|json <normalize-object-arg>|sse <data-blocks|responses-records|responses-result|responses-message>|harness <inspect|apply|presets|preset> [args...]|path <assert [allow-missing|existing]>|perf <startup>|redact|sha256|sha256-base64url|aci <list|read|view|view-json|write|search|search-json|find-json|glob|apply-patch|parse-patch>>".to_string()),
    }
}
