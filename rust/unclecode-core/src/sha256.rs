...
pub fn sha256_base64url(input: &str) -> String {
    sha256_base64url_bytes(input.as_bytes())
}

pub fn sha256_base64url_bytes(input: &[u8]) -> String {
    const TABLE: &[u8; 64] = b"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_";
    let bytes = sha256_digest(input);
...
mod tests {
    ...
    #[test]
    fn hashes_pkce_base64url_vectors() {
        assert_eq!(
            sha256_base64url(""),
            "47DEQpj8HBSa-_TImW-5JCeuQeRkm5NMpJWZG3hSuFU"
        );
        assert_eq!(
            sha256_base64url("abc"),
