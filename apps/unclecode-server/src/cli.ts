...
async function main(): Promise<void> {
  ...
  const { adapter } = createPersistentRuntimeAdapter({ rootDir: getSessionStoreRoot(process.env) });
  const { url, token } = await startServer({ port, host, handlers: makeControlRoomHandlers({ adapter }), insecure });
